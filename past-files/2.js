...
(function () {
    ...
    function ensureBanner(win) {
      try {
        ...
        hint.textContent = 'Apply to set rack. Empty input → empty rack. Missing tiles conjured.';
    ...
    function hookWindow(win) {
      try {
        ...
        win.W7 = function (bag) {
          try {
            // Reseed per bag from last applied value; ignore live edits
            if (!desiredByBag.has(bag)) desiredByBag.set(bag, getDesiredFromBanner(win));
          ...
          win.$6 = function (rackCtx, bag /*, valueString, playerIndex */) {
            try {
              if (bag && typeof bag === 'object') {
                ...
                const fractions = computeFractionsForDesired(bag, desired, props, desired ? desired.length : 0);
