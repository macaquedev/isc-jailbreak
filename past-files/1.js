    ...
    function hookWindow(win) {
      try {
        ...
        if (typeof orig$6 === 'function' && !win.__ISC_TILE_PICKER_$6__) {
          win.$6 = function (rackCtx, bag /*, needed or valueString, ... */) {
            try {
              if (bag && typeof bag === 'object') {
                // Build explicit rack string from the last applied desired input
                const desired = getDesiredFromBanner(win);
                const overrideStr = (desired && desired.length)
                  ? String.fromCharCode.apply(null, desired)
                  : '';
                // Force this rack content exactly; $6 will fill remaining slots with empties
                arguments[2] = overrideStr;
                // Clear queue so any subsequent R7 fill in this same flow won't add extra tiles
                desiredByBag.set(bag, []);
                epochByBag.set(bag, globalInputEpoch);
                lastTextByBag.set(bag, getBannerText(win));
                activeRefillBags.add(bag);
                const result = orig$6.apply(this, arguments);
                activeRefillBags.delete(bag);
                return result;
              }
            } catch(_e) {}
